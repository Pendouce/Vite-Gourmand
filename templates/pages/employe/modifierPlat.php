...
<?php /** @var array $allergenesDuPlatIds */ ?>
...
<div class="contenue-page">
  ...
  <div>
    <a class="flex gap-2 items-center border border-primary/50 bg-primary/70 text-fond-carte rounded-2xl p-4 text-lg font-medium w-fit" href="/detailPlat?id=<?= $plat->getPlatId() ?>">
  ...
  <div class="cart-form">
    <form class="grid grid-cols-2 gap-4" action="/modifierPlat" method="post" enctype="multipart/form-data">
      <input type="hidden" name="csrfToken" id="csrfToken" value="<?= $csrfToken ?>">
      <input type="hidden" name="id" value="<?= $plat->getPlatId() ?>">
      <div class="col-span-2 flex flex-col">
        <label for="titre">Titre</label>
        <input class="grand-input w-2/3" type="text" name="titre" value="<?= htmlspecialchars($plat->getTitre()) ?>">
      </div>

      <div class="col-span-2">
        <label id="imgPlat" for="image_plat">Image du plat</label>
  
        <label class="grand-input w-2/3 h-60 text-texte/50 flex flex-col items-center justify-center relative overflow-hidden" for="image_plat" id="igmLabel">
          ...
          <img class="imgHtml absolute inset-0 w-full h-full object-cover opacity-90" src="<?= htmlspecialchars($plat->getImagePlat())?>" alt="Image <?= htmlspecialchars($plat->getTitre()) ?>">
        </label>
      </div>

      <div class="col-span-2 flex flex-col">
        <label for="description_plat">Description</label>
        <textarea name="description_plat" class="grand-input w-xs md:w-2/3" rows="6"><?= htmlspecialchars($plat->getDescriptionPlat()) ?></textarea>
      </div>
      ...
      <div class="col-span-1 flex flex-col">
        ...
        <select class="grand-input py-2" name="type_id">
          <?php foreach($typeDePlat as $type): ?>
            <option value="<?= $type->getTypeId() ?>" <?= $type->getTypeId() == $plat->getTypeId() ? 'selected' : '' ?>>
              <?= $type->getLibelle() ?>
            </option>
      ...
      <div class="flex justify-between items-center gap-8">
        ...
        <label class="pl-8">
          ...
          <input class="peer appearance-none togglePlatInput" type="checkbox" name="plat_actif" value="<?= $plat->isPlatActif() ?>" data-id="<?= $plat->getPlatId() ?>" <?= $plat->isPlatActif() == 1 ? 'checked' : '' ?>>
      ...
      <div class="col-span-2 flex flex-col">
        ...
        <div class="flex flex-wrap gap-3 pt-4">
          <?php foreach($allergenes as $allergene): ?>
            ...
            <?= in_array($allergene->getAllergeneId(), $allergenesDuPlatIds) ? 'checked' : ''?>>
            </label>
          <?php endforeach; ?>
        </div>
      </div>
